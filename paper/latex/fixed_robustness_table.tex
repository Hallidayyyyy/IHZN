\begin{table}[htb]
\centering
\caption{PP-AT Relative to Fixed TTL Under Four Environments}
\label{tab:fixed_robustness}
\begingroup\small\setlength{\tabcolsep}{4pt}
\begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}lrrrrr@{}}
\toprule
Environment & TTL & MSG reduction (\%) & NTC change (\%) & MSG/NTC reduction (\%) & Seed rate \\
\midrule
...
Each comparison uses 20 paired seeds (79001--79020). Positive reductions indicate lower communication overhead with PP-AT. Negative NTC changes indicate fewer completed tasks. Seed rate is the fraction satisfying $\mathrm{NTC}_{\mathrm{PPAT}}\geq0.99\,\mathrm{NTC}_{\mathrm{Fixed}}$. The four cases were selected before this supplement. Eighty archived PP-AT runs are paired with 400 new fixed-TTL runs. These results are based on the four selected environments, and do not include all ten environments used in the earlier AT comparison.
\end{minipage}
\endgroup
\end{table}
